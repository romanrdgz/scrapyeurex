...
def plot_open_interest(df: pd.DataFrame, t: datetime, ticker: str):
    '''
    ...
    fig, ax = plt.subplots()
    ax.barh(strikes-width/2, call_oi, width, color='blue',      align='center', label='Calls')
    ax.barh(strikes-width/2, call_oi_diff, width, color='limegreen', align='center', label='Calls', left=call_oi)
    ax.barh(strikes+width/2, put_oi,  width, color='red',       align='center', label='Puts')
    ax.barh(strikes+width/2, put_oi_diff,  width, color='limegreen', align='center', label='Puts',  left=put_oi)
    ax.set(yticks=strikes_ticks, ylim=[min_strike-(max_strike-min_strike)*0.1, max_strike+(max_strike-min_strike)*0.1])
